...
def query4():
    return pd.read_sql_query('''
select
    avg(storage_bytes + stage_bytes + failsafe_bytes) / power(10, 8) as billable_gb
from
...
select
    date_trunc(month, usage_date) as usage_month,
    avg(storage_bytes + stage_bytes + failsafe_bytes) / power(10, 8) as billable_gb,
    avg(storage_bytes) / power(10, 8) as Storage_GB,
    avg(stage_bytes) / power(10, 8) as Stage_GB,
    avg(failsafe_bytes) / power(10, 8) as Failsafe_GB
from
    account_usage.storage_usage
group by
    1
...
def show_storage_and_job_details():
    ...
    fig5 = px.bar(query12(), x="USAGE_MONTH", y = ["STORAGE_GB", "STAGE_GB", "FAILSAFE_GB"], color_discrete_sequence=[
                 "orange", "red", "green", "purple"])
...
def main_page():
    ...
    st.title("\n")
    st.subheader("This is the Home page of the dashboard. For accessing CostNomics section please select the other tab shown above.")
